...
export function HomeofficePage() {
  ...
  const WEEKDAYS = [t('homeoffice.weekdays.mon'), t('homeoffice.weekdays.tue'), t('homeoffice.weekdays.wed'), t('homeoffice.weekdays.thu'), t('homeoffice.weekdays.fri')];
  const WEEKDAY_SHORT = [t('homeoffice.weekdaysShort.mon'), t('homeoffice.weekdaysShort.tue'), t('homeoffice.weekdaysShort.wed'), t('homeoffice.weekdaysShort.thu'), t('homeoffice.weekdaysShort.fri')];

  useEffect(() => { loadSettings(); }, []);

  async function loadSettings() { const res = await api.getHomeofficeSettings(); if (res.success && res.data) setSettings(res.data); setLoading(false); }
  ...
  async function toggleWeekday(day: number) {
    if (!settings) return;
    const next = settings.pattern.weekdays.includes(day) ? settings.pattern.weekdays.filter(d => d !== day) : [...settings.pattern.weekdays, day].sort();
    await api.updateHomeofficePattern(next);
    setSettings({ ...settings, pattern: { weekdays: next } });
    toast.success(t('homeoffice.patternUpdated'));
  }
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 0; i < startDow; i++) { const d = new Date(year, month, 1 - startDow + i); days.push({ date: d, inMonth: false, isHo: false, isToday: false }); }
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-8">
      <motion.div variants={itemVariants}>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3"><House weight="fill" className="w-7 h-7 text-sky-600" />{t('homeoffice.title')}</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1">{t('homeoffice.subtitle')}</p>
      </motion.div>
      ...
      {isHoToday && (
        <motion.div variants={itemVariants} className="card bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800 p-4">
          <div className="flex items-center gap-3"><House weight="fill" className="w-6 h-6 text-sky-600 dark:text-sky-400" /><div>
            <p className="font-semibold text-sky-800 dark:text-sky-200">{t('homeoffice.todayBannerTitle')}</p>
            <p className="text-sm text-sky-600 dark:text-sky-400">{t('homeoffice.todayBannerDesc', { slot: settings?.parkingSlot?.number })}</p>
          </div></div>
        </motion.div>
      )}
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="stat-card">
          <div className="flex items-start justify-between"><div><p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('homeoffice.thisWeek')}</p><div className="mt-2 flex items-baseline gap-2"><span className="stat-value text-sky-600 dark:text-sky-400">{hoThisWeek}</span><span className="text-gray-500 dark:text-gray-400">{t('homeoffice.homeOfficeDays')}</span></div></div>
          <div className="w-12 h-12 bg-sky-100 dark:bg-sky-900/30 rounded-xl flex items-center justify-center"><Calendar weight="fill" className="w-6 h-6 text-sky-600 dark:text-sky-400" /></div></div>
        </div>
        {settings?.parkingSlot && (
          <div className="stat-card">
            <div className="flex items-start justify-between"><div><p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('homeoffice.yourParkingSlot')}</p>
            <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{t('dashboard.slot')} <span className="font-bold text-lg text-gray-900 dark:text-white">{settings.parkingSlot.number}</span> {t('homeoffice.slotAvailableOnHo')}</p></div>
            <div className="w-12 h-12 bg-emerald-100 dark:bg-emerald-900/30 rounded-xl flex items-center justify-center"><Briefcase weight="fill" className="w-6 h-6 text-emerald-600 dark:text-emerald-400" /></div></div>
          </div>
        )}
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-8">
          <motion.div variants={itemVariants} className="card p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><CalendarCheck weight="fill" className="w-5 h-5 text-sky-600" />{t('homeoffice.regularDays')}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t('homeoffice.regularDaysDesc')}</p>
            <div className="grid grid-cols-5 gap-2">
              {WEEKDAYS.map((name, i) => {
                const active = settings?.pattern.weekdays.includes(i);
                return (
                  <motion.button key={i} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => toggleWeekday(i)}
                    className={`flex flex-col items-center gap-1 py-3 px-2 rounded-xl border-2 transition-all font-medium ${active ? 'bg-sky-100 dark:bg-sky-900/40 border-sky-400 dark:border-sky-600 text-sky-700 dark:text-sky-300' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                    <span className="text-xs">{WEEKDAY_SHORT[i]}</span><span className="text-sm">{name}</span>{active && <House weight="fill" className="w-4 h-4 mt-1" />}
                  </motion.button>
                );
              })}
            </div>
        ...
        <motion.div variants={itemVariants} className="card p-6">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            <div className="text-center text-xs font-semibold text-gray-400 dark:text-gray-500 py-2">{t('homeoffice.weekdaysShort.sun')}</div>
            {calendarDays.map((day, i) => {
              const isWeekend = day.date.getDay() === 0 || day.date.getDay() === 6;
              return (
                <div key={i} className={`aspect-square flex items-center justify-center rounded-lg text-sm font-medium transition-all ${!day.inMonth ? 'text-gray-300 dark:text-gray-700' : day.isToday ? 'ring-2 ring-primary-500 ring-offset-1 dark:ring-offset-gray-900' : ''} ${day.isHo && day.hoType === 'pattern' ? 'bg-sky-200 dark:bg-sky-800/50 text-sky-800 dark:text-sky-200' : day.isHo && day.hoType === 'single' ? 'bg-emerald-200 dark:bg-emerald-800/50 text-emerald-800 dark:text-emerald-200' : isWeekend && day.inMonth ? 'text-gray-400 dark:text-gray-500' : day.inMonth ? 'text-gray-700 dark:text-gray-300' : ''}`}>{day.date.getDate()}</div>
              );
            })}
          </div>
          <div className="flex flex-wrap gap-4 mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500 dark:text-gray-400">
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-sky-200 dark:bg-sky-800/50" /><span>{t('homeoffice.legendRegular')}</span></div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-emerald-200 dark:bg-emerald-800/50" /><span>{t('homeoffice.legendSingle')}</span></div>
            <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm ring-2 ring-primary-500" /><span>{t('homeoffice.legendToday')}</span></div>
          </div>
        </motion.div>
      </div>
    </motion.div>
