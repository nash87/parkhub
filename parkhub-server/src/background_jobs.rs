...
pub fn start_background_jobs(state: Arc<RwLock<AppState>>, email: Option<EmailService>) {
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            if let Err(e) = run_auto_release(&s1, &e1).await { warn!("Auto-release error: {}", e); }
        }
    });
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            if let Err(e) = run_reminders(&s2, &e2).await { warn!("Reminder error: {}", e); }
        }
    });
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            if let Err(e) = cleanup_expired_waitlist(&s3).await { warn!("Waitlist cleanup error: {}", e); }
        }
    });
...
async fn run_auto_release(state: &Arc<RwLock<AppState>>, email: &Option<EmailService>) -> anyhow::Result<()> {
    ...
    for booking in &bookings {
        if booking.status != parkhub_common::BookingStatus::Confirmed { continue; }
        if booking.checked_in_at.is_some() { continue; }
        ...
        if elapsed >= auto_release_minutes {
            ...
            info!("Auto-released booking {} (no check-in after {} min)", booking.id, auto_release_minutes);

            if let Some(ref es) = email {
                if let Ok(Some(user)) = sg.db.get_user(&booking.user_id.to_string()).await {
                    let _ = es.send_auto_release_notification(&user.email, &user.name, booking.slot_number.as_deref().unwrap_or("?"), booking.lot_name.as_deref().unwrap_or("Lot")).await;
                }
            }

            // Notify waitlist
            let date = booking.start_time.format("%Y-%m-%d").to_string();
            let wl = sg.db.list_waitlist_by_lot(&booking.lot_id.to_string(), Some(&date)).await.unwrap_or_default();
            if let Some(first) = wl.first() {
                if !first.notified {
                    if let Some(ref es) = email {
                        if let Ok(Some(wu)) = sg.db.get_user(&first.user_id.to_string()).await {
                            let _ = es.send_waitlist_notification(&wu.email, &wu.name, booking.lot_name.as_deref().unwrap_or("Lot"), &date).await;
                        }
                    }
...
async fn run_reminders(state: &Arc<RwLock<AppState>>, email: &Option<EmailService>) -> anyhow::Result<()> {
    let es = match email { Some(e) => e, None => return Ok(()) };
    let sg = state.read().await;
    let bookings = sg.db.list_bookings().await?;
    let now = Utc::now();
    for b in &bookings {
        if b.status != parkhub_common::BookingStatus::Confirmed { continue; }
        ...
        if (25..=30).contains(&mins) {
            if let Ok(Some(user)) = sg.db.get_user(&b.user_id.to_string()).await {
                let _ = es.send_booking_reminder(&user.email, &user.name, b.slot_number.as_deref().unwrap_or("?"), b.lot_name.as_deref().unwrap_or("Lot"), &b.start_time.to_rfc3339()).await;
            }
        }
    }
    Ok(())
...
async fn cleanup_expired_waitlist(state: &Arc<RwLock<AppState>>) -> anyhow::Result<()> {
    ...
    for e in &entries {
        if e.date < today { let _ = sg.db.delete_waitlist_entry(&e.id.to_string()).await; }
    }
    Ok(())
}
